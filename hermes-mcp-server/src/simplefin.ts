...
export class SimpleFinClient {
   ...
   */
  static async claimSetupToken(tokenBase64: string): Promise<string> {
    try {
   ...
   */
  static async fetchAccountsAndTransactions(accessUrl: string, daysBack: number = 90): Promise<{ accounts: SimpleFinAccount[]; transactions: Transaction[] }> {
    try {
