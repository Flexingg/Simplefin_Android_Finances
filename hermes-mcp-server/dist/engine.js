export class FinanceEngine {
     ...
     */
    static applyRules(transactions, rules) {
        let count = 0;
     ...
     */
    static calculateSummary(transactions, budgets, incomeCategoryName = 'Income') {
        const now = new Date();
    ...
    static matchesCategory(item, targetCat, targetSub) {
     ...
     */
    static getHistoricalTrends(transactions, monthsBack = 6) {
        const trends = [];
        const now = new Date();
     ...
     */
    static getDailyHeatmap(transactions, days = 90) {
        const heatmap = {};
        ...
        Object.values(heatmap).forEach(entry => {
            if (entry.totalSpent === 0)
                entry.level = 0;
            else if (entry.totalSpent < 35)
                entry.level = 1;
            else if (entry.totalSpent < 100)
                entry.level = 2;
            else if (entry.totalSpent < 250)
                entry.level = 3;
            else
                entry.level = 4;
        });
     ...
     */
    static simulateDebtPayoff(debts, monthlyExtraBudget = 200) {
        if (debts.length === 0) {
            return { snowballMonths: 0, avalancheMonths: 0, snowballInterest: 0, avalancheInterest: 0 };
        }
        const runSimulation = (sortStrategy) => {
